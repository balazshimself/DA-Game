...
//     document.getElementById('endTurn')!.removeEventListener('click', () => {});
  
//     document.getElementById('resetGame')!.removeEventListener('click', () => {});
...
export class App {
  ...
  render(shouldFlip: boolean = false) {
    const size = 30;
    function posX(v: Vertex): number {
      return 200 + size * (3 / 2 * v.x);
    }
    ...
    // Create the animateTransform element for smooth rotation
    const boardGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    const centerX = 200;
    const centerY = 200;

    // boardGroup.setAttribute()

    if (shouldFlip) {
      // Create the animateTransform element for smooth rotation with easing
      const animateTransform = document.createElementNS('http://www.w3.org/2000/svg', 'animateTransform');
      animateTransform.setAttribute('attributeName', 'transform');
      animateTransform.setAttribute('type', 'rotate');
      animateTransform.setAttribute('dur', '0.5s');
      animateTransform.setAttribute('fill', 'freeze');
      animateTransform.setAttribute('calcMode', 'spline'); // Use spline for easing
      animateTransform.setAttribute('keyTimes', '0;1');
      animateTransform.setAttribute('keySplines', '0.42 0 0.58 1'); // Ease-in-out effect

      // Determine rotation based on the current player
      if (this.#currentPlayer === 'red') {
        animateTransform.setAttribute('from', `0 ${centerX} ${centerY}`);
        animateTransform.setAttribute('to', `180 ${centerX} ${centerY}`);
      } else {
        animateTransform.setAttribute('from', `180 ${centerX} ${centerY}`);
        animateTransform.setAttribute('to', `0 ${centerX} ${centerY}`);
      }

      // Add the animation to the boardGroup
      boardGroup.appendChild(animateTransform);
      this.#svg.appendChild(boardGroup);

      // Begin the animation immediately
    ...
    // Create UI group for non-rotated elements
    ...
    if (!energyText) {
      ...
      energyText.setAttribute('fill', '#E5E7EB');
      energyText.setAttribute('font-size', '16');
      uiGroup.appendChild(energyText);
    }
    const energy = this.#currentPlayer === 'blue' ? this.#blue.energy : this.#red.energy;
    energyText.textContent = `${this.#currentPlayer}'s turn - Energy: ${energy}`;
  }
  ...
  handleEdgeClick(e: Edge) {
    ...
    if (childId === opponentState.network._id) {
      alert(`${this.#currentPlayer.toUpperCase()} wins!`);
    }
  ...
  collectEnergy(player: 'blue' | 'red') {
    ...
    function hasDescendantFactory(point: Point): boolean {
      if (point._isFactory) return true;
      for (const child of point._children) {
        if (hasDescendantFactory(child)) return true;
      }
      return false;
